...
int main(int argc, char *argv[])
{
    ...
    // retrieve current env's process number from MPI Context
    MPI_Comm_size(MPI_COMM_WORLD, &np);

    // retrieve current processor's rank id from MPI Context 
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);

    if (rank == 0)
        printf("Number of process: %d\n", np);

    // np means number of processors MPI own 
    // divide bufsize by np get each processor process how many bytes in buffer      
    bufsize /= np;
    size /= np;
    ...
    // Terminate MPI and release MPI Context resources
    MPI_Finalize();
