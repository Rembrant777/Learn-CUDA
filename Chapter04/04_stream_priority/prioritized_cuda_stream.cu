...
void Operator::print_time() {
    ...
    printf("stream %2d - priority %2d - elapsed %.3f ms \n", _index, get_stream_priority(),
                elapsed_time_msed);
}

// here we add a get stream priority 
int Operator::get_stream_priority() {
    int priority; 

    cudaError_t err = cudaStreamGetPriority(stream, &priority); 

    if (err != cudaSuccess) {
        printf("#get_stream_priority failed to get priority value, err msg %s\n",
                    cudaGetErrorString(err)); 
        priority = -999;                     
    } else {
        printf("#get_stream_priority retrieve priority value %d", priority); 
    }
...
int main(int argc, char* argv[])
{
    ...
    // here we create an index to iterate given operator nums from [0 to num_operator - 1]
    // since we already set priority 
    // every time we execute the #async_operation 
    // block will be appplined and execute each operation concurrently
    // and the priority of each operation should obey the high priority first be executed. 
    for (int i = 0; i < num_operator; i++) {
        int offset = i * size / num_operator;
