...
void init_buffer(float *a, const int size); 
...
private:
    ...
    static void CUDART_CB Callback(cudaStream_t stream, cudaError_t status, void *useData);     
    void print_timer(); 
...
void Operator::print_timer() 
{
    ...
    printf("stream %2d - elasped %.3f ms\n", _index, elapsed_time_msed); 
}
