...
private:
    ...
    // this Callback is function pointer 
...
public:
    ...
    void print_kernel_time_via_cpu_timer();                             
    void print_kernel_time_via_cuda_event(); 
...
void Operator::CUDART_CB Callback(cudaStream_t stream, cudaError_t status, void *userData)
{
    ...
    this_->print_kernel_time_via_cpu_timer(); 
    this_->print_kernel_time_via_cuda_event(); 
}

void Operator::print_kernel_time_via_cpu_timer()
{
    // here invoke stop timer 
    sdkStopTimer(&p_timer); 
    float elapsed_time_msed = sdkGetTimerValue(&p_timer); 
    printf("#print_kernel_time_via_cpu_timer \nstream %2d - elapsed %.3f ms\n", 
            index, elapsed_time_msed); 
}

void Operator::print_kernel_time_via_cuda_event() 
{
    // here retrieve elapsed time in milliseconds via cuda events of {start, stop}
    float elapsed_time_msed = 0.f; 
    cudaEventElapsedTime(&elapsed_time_msed, start, stop); 
    printf("#print_kernel_time_via_cuda_event \nstream %2d - elapsed %.6f ms\n", 
            index, elapsed_time_msed); 
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < num_operator; i++)
        ls_operator[i].print_kernel_time_via_cpu_timer(); 
