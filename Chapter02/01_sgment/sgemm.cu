/**
 cuda_runtime.h and device_launch_parameters.h are cuda library header files which locate in 
 /usr/local/cuda-9.2/include/cuda_runtime.h
 /usr/local/cuda-9.2/include/device_launch_parameters.h
*/
#include "cuda_runtime.h"
#include "device_launch_parameters.h"
#include <stdio.h>

// benchmark lib locates in
...
void performance_estimation(
    void(*sgemm)(const float *, const float *, float *, int, int, int, float, float),
    const float *A, const float *B, float *C, int N, int M, int K, float alpha, float beta) 
{
    ...
    // Waits for GPU operation finish and record the time 
    sdkStopTimer(&timer); 
...
int main() 
{
    ...
    // terminates allocated host memory space
    free(A);
    free(B); 
    free(C); 
