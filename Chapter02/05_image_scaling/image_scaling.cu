...
__global__
void createResizedImage(unsigned char *imageScaledData, int scaled_width, 
                        float scale_factor, cudaTextureObject_t texObj) 
{
    ...
    imageScaledData[index] = tex2D<unsigned char>(texObj, (float)(tidX*scale_factor),
...
int main(int argc, char * argv[]) 
{
    ...
    // here we create a channel description to be used while linking to the tecture 
    cudaArray* cu_array; 
    ...
    // here we get height and width of the current image
    get_pgmPpmParamsx(inputStr, &height, &width); 
    data = (unsigned char*) malloc(height * width * sizeof(unsigned char)); 
    printf("\n Reading image width height and width [%d][%d]", height, width); 
    // loading an image as the 'inputimage'
    ...
    // after retriving the scaled height and width 
    // allocate correpsoind scaled image space on cuda memory 
    ...
    returnValue = (cudaError_t) (returnValue | 
                    cudaMemcpy(scaled_data, d_scaled_data, 
