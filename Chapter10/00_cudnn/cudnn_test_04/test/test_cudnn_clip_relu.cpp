...
TEST(TestCudnnActivationDescriptor, CreateClippedReluActivationDescriptor)
{
    ...
    double coef                      = 0.0; 
    EXPECT_EQ(mode, CUDNN_ACTIVATION_CLIPPED_RELU);
...
TEST(TestCudnnActivationDescriptor, ApplyClippedReluActivationForward)
{
    EXPECT_EQ(1, 1);
}


TEST(TestCudnnActivationDescriptor, ApplyClippedReluActivationBackforward)
{
    EXPECT_EQ(1, 1);
}