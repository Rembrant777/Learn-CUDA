...
class CudnnTanhActivationDescriptor {
public:
    CudnnTanhActivationDescriptor(cudnnActivationMode_t mode, cudnnNanPropagation_t nan_prop, float coef): 
                activate_mode_(mode), nan_prop_(nan_prop), coef_(coef) {
    }

    ~CudnnTanhActivationDescriptor() {
    }
...
protected:
    static void SetUpTestCase() {
        instance = new CudnnTanhActivationDescriptor(CUDNN_ACTIVATION_TANH, CUDNN_PROPAGATE_NAN, 0.0);
    ...
    static CudnnTanhActivationDescriptor* instance;
};

CudnnTanhActivationDescriptor* CudnnActivationTest::instance = nullptr;


// enumeration: CUDNN_ACTIVATION_TANH
TEST_F(CudnnActivationTest, CreateTanhActivationDescriptor)
{
