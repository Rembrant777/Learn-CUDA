...
__device__
float clip(float prediction, float epsilon = 1e-12)
{
...
*/
__global__
void softmax_loss_kernel(float *reduced_loss, float *precit, float *target, float *workspace, int batch_size, int num_outputs)
{
    ...
    // each thread calculate entropy for each data and 
    ...
    // then, we do reduction the result to calculate lossing using 1 thread block 
