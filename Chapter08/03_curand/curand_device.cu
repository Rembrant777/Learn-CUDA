...
__global__
void setup_kernel(curandState_t *state)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x; 
    // each thread gets same seed 
    // a different sequence number, on offset
    curand_init(2024UL, idx, 0, &state[idx]); 
}

__global__
void generate_kernel(unsigned int *generate_out, curandState_t* state)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x; 
...
__global__
void generate_uniform_kernel(float *generate_out, curandState_t* state)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x; 
