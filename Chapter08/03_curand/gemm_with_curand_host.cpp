...
template <typename T>
// here adopted the type deduce and SFINAE(Substitution Failure Is Not An Error)
// this grammar maintains that only curand<float> will work as expected, 
// otherwise this template will not be compiled and initialized which means cannot be used 
typename std::enable_if<(std::is_same<T, float>::value), float>::typename
...
int main()
{
    ...
    curandCreateGenerator(&curand_gen, CuRNAD_RNG_PSEUDO_DEFAULT); 
    curandSetPseudoRandomGeneratorSeed(curand_gen, 2024UL); 

    // create cublas handler 
    stat = cublasCreate(&dublas_handle); 
    ...
    cublasGemmAlgo_t gemm_algo = (tensor_core) ? CUBLAS_GEM_DEFAULT_TENSOR_OP : CUBLAS_GEMM_DEFAULT; 
    ...
    // enable tensor-core operation when it is possible 
    ...
    std::cout << std::setw(4) << "Elapsed Time: " << cudaElapsedTime << " ms" << std::endl; 
