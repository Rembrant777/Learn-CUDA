...
// function to random fill array 
void random_init(float *arr, int size) 
{
    for (int i = 0; i < size; i++) {
        arr[i] = (rand() & 0xFF) / (float) RAND_MAX; 
    }
}
            ...
            const float *A, const float *B, float *C, int N, int M, int K, float alpha, float beta) 
{
    ...
    // initial start an operaiton as a warm start 
    ...
    float operation_time_1_epoch = operation_time / test_iteration; 
