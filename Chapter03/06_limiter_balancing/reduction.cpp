        ...
        float *d_outPtr, float *d_inPtr, int size)
{
    ...
    float elaspsed_time_msed = sdkGetTimerValue(&timer) / (float) test_iter; 
    float bandwidth = size * sizeof(float) / elaspsed_time_msed / 1e6; 
    printf("Time= %.3f msec, bandwith= %f GB/s\n", elaspsed_time_msed, bandwidth); 
...
void init_input(float *data, int size) 
{
    for (int i = 0; i < size; i++) {
        // keep the numbers small so we didn't get truncation error in the sum 
