...
__inline__ __device__
float block_reduce_sum(thread_block block, float val)
{
    // apply shared memory space for 32 partial sums s
...
void reduction_kernel(float *g_out, float *g_in, unsigned int size)
{
    ...
    for (int i = idx_x; i < size; i += blockDim.x * gridDim.x * NUM_LOAD) {
        ...
        // iterate for NUM_LOAD times to accumulate middle result together 
