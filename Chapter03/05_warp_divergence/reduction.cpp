...
// reduction function define header file
#include "reduction.h"
...
void message()
{
    puts("Invalid reduction result !! 0-1 are available."); 
    exit(EXIT_FAILURE); 
}
...
int main(int argc, char *argv[])
{
    ...
    // init applied input data array pointed by h_inPtr 
    init_input(h_inPtr, size); 
    result_host = get_cpu_result(h_inPtr, size); 

    // prepare GPU input data array space and output data array 
